...
# Credit goes to https://github.com/cnelson/skydetector
...
def mask(b, image, color=[0, 0, 255]):
    result = image.copy()
...
def detect_sky(image) -> Tuple[bool, str]:
    ...
    if no_sky_region(bopt, image.shape[0] / 30, image.shape[0] / 4, 5):
        _LOG.info("No sky detected")
        detected = False

    if partial_sky_region(bopt, image.shape[1] / 2):
        bnew = refine_sky(bopt, image)
        image = mask(bnew, image)
    else:
        detected = False
